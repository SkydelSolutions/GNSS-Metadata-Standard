...
namespace GnssMetadata
{
	...
	public:
		...
		{
			return _countWords > 1 && _sizeWord > 0 &&  _lumplist.size() > 0;
		}
		 ...
		 */
		void CountWords( const size_t count )
		{
			_countWords = count;
		}
		 ...
		 */
		void SizeWord( const size_t sizeWord )
		{
			_sizeWord = sizeWord;
		}
		 ...
		 */
		void Padding( const WordPadding bits)
		{
			_padding = bits;
		}
		 ...
		 * Sets the word shift.
		 */
		void Shift( const WordShift shift)
		{
			_shift = shift;
		}
		/**
		 * Gets the word shift.
		 */
		WordShift Shift( ) const
		{
			return _shift;
		}

		const GnssMetadata::LumpList& Lumps( ) const
		{
			return _lumplist;
		}
		GnssMetadata::LumpList& Lumps( )
		{
			return _lumplist;
		}
